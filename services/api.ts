...
import { emitAuthUnauthorized } from "@/lib/authEvents";
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (message.includes("network") || message.includes("failed to fetch") || message.includes("internet")) {
        throw new Error("Internet muammosi: tarmoqni tekshirib qayta urinib ko'ring.");
      }
    }
    throw error;
  } finally {
    ...
    try {
      ...
      if (parsed) {
        ...
        errorMessage = Array.isArray(detail)
          ? detail[0]?.msg || parsed?.message || parsed?.error || raw
...
export async function loginUser(email: string, password: string): Promise<LoginResponse> {
  return runAuthRequestWithRetry((timeoutMs) =>
    fetchApi<LoginResponse>("/auth/login", {
      ...
      body: JSON.stringify({ email: sanitizeEmail(email), password: password.trim() }),
    }, false, { timeoutMs })
  );
}
...
export async function requestPhoneOtp(payload: PhoneOtpRequestPayload): Promise<PhoneOtpSendResponseApi> {
  return runAuthRequestWithRetry((timeoutMs) =>
    fetchApi<PhoneOtpSendResponseApi>("/auth/phone/request", {
      ...
      body: JSON.stringify({
        ...payload,
        name: payload.name ? sanitizeName(payload.name) : undefined,
      }),
    }, false, { timeoutMs })
  );
}
  ...
  return runAuthRequestWithRetry((timeoutMs) =>
    fetchApi<LoginResponse>("/auth/phone/verify", {
      ...
      body: JSON.stringify({
        ...payload,
        name: payload.name ? sanitizeName(payload.name) : undefined,
        code: String(payload.code || "").replace(/\D/g, "").slice(0, 6),
      }),
    }, false, { timeoutMs })
  );
}
...
export async function verifyAuthToken(): Promise<{ user_id: number; role: string }> {
  return fetchApi<{ user_id: number; role: string }>("/auth/verify", {
    method: "POST",
  }, true, { timeoutMs: 8000 });
}
...
export async function getUserAppointments(_userId: number): Promise<UserAppointmentApi[]> {
...
export async function getAppSupportConfig(): Promise<AppSupportConfigApi> {
  return fetchApi<AppSupportConfigApi>("/app/support-config", {}, true);
}
...
export async function getUserNotifications(): Promise<UserNotificationApi[]> {
  return fetchApi<UserNotificationApi[]>("/user/notifications", {}, true);
}
